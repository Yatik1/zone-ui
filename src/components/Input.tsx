...
function Input() {
  ...
  const { setMessages, messages, setMessage: newMessage, setNewChat } =
    useMessage() as MessageContextProps;

  const [message, setMessage] = useState("");
  const [selectedResponse, setSelectedResponse] = useState<"short" | "detailed" | undefined>("short");
  ...
  async function fileSubmit() {
    try {
      ...
      newMessage({ fileName: file?.name, message });
  ...
  if (!browserSupportsSpeechRecognition) {
    return (
      <div className="text-red-600 text-sm font-medium px-2 py-1">
  ...
  return (
    <div className="relative flex border h-fit bg-white border-gray-300 rounded-xl px-4 py-4 mb-1 flex-1 items-center justify-center gap-2 shadow-md">
      <div className="w-full h-full flex flex-col items-start justify-center gap-2">
        {file && <FileSegment fileName={file.name} remove={removeFile} />}

        {listening && (
          <div className="text-sm flex items-center justify-center px-2 py-0.5 bg-gradient-to-b from-stone-700 to-stone-500 text-white tracking-tight rounded-sm shadow-sm shadow-black">
            Listening..
          </div>
        )}
        ...
        <div className={`flex flex-1 w-full items-center justify-center gap-2`}>
          <div
            className="flex items-center justify-center p-1 rounded-full border border-gray-300 cursor-pointer"
              ...
              <>
                <button
                  className={`flex items-center justify-start gap-1 ${
                    selectedResponse === "short"
                      ? "bg-orange-300 text-orange-900 border border-orange-600"
                ...
                <button
                  className={`flex items-center justify-center gap-1 ${
                    selectedResponse === "detailed"
                      ? "bg-emerald-300 text-emerald-900 border border-emerald-600"
                      : "bg-gray-200 text-gray-800"
                  } text-sm px-3 py-1 rounded-full cursor-pointer`}
                  onClick={() => {
          ...
          <div className="flex items-end justify-end gap-1">
            ...
            <button
              className={`flex-shrink-0 rounded-full text-white flex items-center justify-center p-1 ${
                disabled && "hidden"
              } ${
                loading
                  ? "bg-gray-400 cursor-not-allowed"
                  : "bg-gradient-to-r from-stone-700 via-stone-600 to-stone-500 cursor-pointer"
              }`}
      ...
      {loading &&
        (!selectedResponse || selectedResponse === "detailed" || isFile) && (
          <motion.div
